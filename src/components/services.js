...
const Services = ({ node }) => {
  return (
    <div className={[servicesStyles.wrapper, "services-wrapper container-fluid"].join(" ")}>
      <div className="row">
        {node.relationships.field_service_card.map((item, index, array) => {
          return (
            <div className={(index == 0 || index % 2 == 0) && array.length % 2 ? "col-12 col-md-6 col-lg-5 offset-lg-1 " + servicesStyles.columnWrapper : (index == 0 || index % 2 == 0) ? "col-12 col-md-6 col-lg-5 offset-lg-1 " + servicesStyles.columnWrapper : "col-12 col-md-6 col-lg-5 " + servicesStyles.columnOddWrapper}>
              <div className={servicesStyles.cardWrapper}>
                ...
                {item.relationships.field_service_image ? item.relationships.field_service_image.localFile ? <div className={index == 2 || index == 3 ? servicesStyles.image + ' services-image ' + servicesStyles.specialImage : servicesStyles.image + " services-image"}><GatsbyImage
                  image={item.relationships.field_service_image ? item.relationships.field_service_image.localFile ? item.relationships.field_service_image.localFile.childImageSharp.gatsbyImageData : "" : ""}
                  alt="img" /></div> : '' : ""}
...
export const fragment = graphql`fragment paragraphServicesParagraph on paragraph__services_paragraph {
  id
  relationships {
    field_service_card {
      field_card_type
