  ...
  return (
    <div className={(node.field_not_homepage && node.field_image_is_right)?`${node.field_colored_custom_class? node.field_colored_custom_class : ""}` + " " + coloredBoxesStyle.notHome + " container-fluid " + coloredBoxesStyle.imageRight : (!node.field_image_is_right && node.field_not_homepage)? `${node.field_colored_custom_class? node.field_colored_custom_class : ""}` + " container-fluid " + coloredBoxesStyle.notHome : "container-fluid " + `${node.field_colored_custom_class? node.field_colored_custom_class : ""}`} id="colored-boxes">
      <div className={["row colored-big-section", coloredBoxesStyle.bigSection].join(" ")}>
        <div className={blogPage? `col-12 col-lg-5 colored-first-section ${coloredBoxesStyle.colLeftPadding} ${coloredBoxesStyle.firstSection}`  : !node.field_not_homepage && node.field_colored_custom_class? "col-10 offset-2 col-lg-5 offset-lg-0 colored-first-section "+ coloredBoxesStyle.colLeftPadding + " " + coloredBoxesStyle.firstSection : !node.field_not_homepage ? "col-10 col-lg-5 "+ coloredBoxesStyle.colLeftPadding + " " + coloredBoxesStyle.firstSection : "col-12  col-lg-5 "+ coloredBoxesStyle.colLeftPadding + " " + coloredBoxesStyle.colRightPadding + " " + coloredBoxesStyle.firstSection}>
          {node.relationships && node.relationships.field_colored_boxes_image && node.relationships.field_colored_boxes_image.localFile && node.relationships.field_colored_boxes_image.localFile.childImageSharp? <div className={[coloredBoxesStyle.image, "colored-image"].join(" ")}><Img alt="img"  fluid={node.relationships.field_colored_boxes_image.localFile.childImageSharp.fluid} /></div> : ''}
        </div>
