...
const ProductLine = ({ node }) => {
    ...
    function addActiveClass(e) {
        document.querySelectorAll(".tab").forEach(Elem => Elem.classList.remove("active"));
        let active = e.target;
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-12">
                        ...
                        {node.field_product_line_title ? <div dangerouslySetInnerHTML={{ __html: node.field_product_line_title.processed }} className={lineStyles.wrapperTitle}></div> : ''}
                        
                    </div>

                    <div style={{ width: "100%" }}>
                        <Slider ref={slider => (slider1.current = slider)} {...TabSliderSetting} className="tab-slider">
                            {node.relationships.field_line_card ? node.relationships.field_line_card.map((item, index) => {
                                return (
                                    <div>
                                        <div className="col-12 p-0">
                                            <div className={lineStyles.tabWrapper}>
                                                {item.field_tab_title ? <div dangerouslySetInnerHTML={{ __html: item.field_tab_title.processed }} onClick={() => slickGoToslide(index)} className={[lineStyles.tab, "line-tab"].join(" ")}></div> : ''}
                                            </div>
                                        </div>
                                    </div>
                                )
                            ...
                            {node.relationships.field_line_card ? node.relationships.field_line_card.map((item, index) => {
                                return (
                                    <div>
                                        ...
                                        <div className={["col-12", lineStyles.cardWrapper].join(" ")}>
                                            {item.field_card_subtitle ? <div dangerouslySetInnerHTML={{ __html: item.field_card_subtitle.processed }} className="subtitle"></div> : ''}
                                            {item.field_line_title ? <div dangerouslySetInnerHTML={{ __html: item.field_line_title.processed }} className={lineStyles.cardTitle}></div> : ''}
                                            <div className={lineStyles.products}>PRODUCTS (<span className={lineStyles.productsNo}>{productCount}</span>)</div>
                                            {item.field_line_description ? <div dangerouslySetInnerHTML={{ __html: item.field_line_description.processed }} className={lineStyles.description}></div> : ''}
                                            <div className={lineStyles.perfect}>PERFECT FOR: {item.relationships.field_line_category.map((item, index, array) => {
                                                return <span className={lineStyles.category}><Link to="#"> {item.name}</Link>{index === array.length - 1 ? '' : ', '}</span>
                                            })}
                                            </div>
                                            {item.relationships.field_line_image.localFile ? <div className={lineStyles.imageWrapper}><Img fluid={item.relationships.field_line_image.localFile.childImageSharp.fluid} /></div> : ''}
                                            {item.field_line_link ? <div><Link to={item.field_line_link.uri} className={["button-link", lineStyles.link].join(" ")}>{item.field_line_link.title}</Link></div> : ''}
                                        </div>
